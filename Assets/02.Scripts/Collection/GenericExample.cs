...
public class GenericExample : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 갑자기 사람 데이터의 요구사항이 Age가 int가 아닌 float값을 담아야 하는 요구사항
...
// 여러가지 타입을 유연하게 사용할 수 있다.
public class Person<T1, T2>      // 2. 그리고 클래스 이름 옆에 <T>를 추가해줌으로써 객체를 생성할 때 이 타입(T)을 정의하게 한다.
{
    public string Name;
